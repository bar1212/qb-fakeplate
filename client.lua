...
local function isBike(vehicle)
    ...
    return class == 8 -- Class 8 is bikes
end
...
lib.onCache('vehicle', function(vehicle)
    if vehicle == nil then
        ...
        if ped == PlayerPedId() then
            flipinstalled = lib.callback.await('bar-fakeplate:IsFlippedInstalled', 500, GetVehicleNumberPlateText(vehicle))
        end
    end
end)
...
RegisterCommand('plateflip', function()
    ...
    if not isPlateHidden(vehicle) then
        TriggerServerEvent('bar-fakeplate:TogglePlate', vehicleNetId, true)
        ...
        if Entity(vehicle).state.plate_index ~= nil then
            ...
            lib.callback('bar-fakeplate:GetPlateIndex', false, function(plateIndex)
                SetVehicleNumberPlateTextIndex(vehicle, plateIndex)
            end, GetVehicleNumberPlateText(vehicle), vehicleNetId)
        end

        TriggerServerEvent('bar-fakeplate:TogglePlate', vehicleNetId, false)
    end
...
RegisterNetEvent('bar-fakeplate:InstallPlateFlipper', function()
    ...
    local isVehicleOwned = lib.callback.await('bar-fakeplate:callback:IsVehicleOwned', false, plate)
    ...
    TriggerServerEvent('bar-fakeplate:SetVehicleDoorsLocked', NetworkGetNetworkIdFromEntity(vehicle), 2)
    
    local position
    local ped = PlayerPedId()
    
    -- Handle both regular vehicles and motorcycles
    if isBike(vehicle) then
        -- For motorcycles, use rear axle position or center
        local boneid = GetEntityBoneIndexByName(vehicle, 'seat_pside_r')
        if boneid ~= -1 then
            position = GetEntityBonePosition_2(vehicle, boneid)
        else
            position = GetEntityCoords(vehicle)
        end
    else
        -- For cars, use platelight bone
    ...
    if lib.progressBar({
        ...
        label = isBike(vehicle) and 'Installing plate flipper on motorcycle...' or 'Installing plate flipper...',
        useWhileDead = false,
        canCancel = true,
        anim = {
            dict = 'mini@repair',
        ...
        TriggerServerEvent('bar-fakeplate:SetVehicleDoorsLocked', NetworkGetNetworkIdFromEntity(vehicle), 1)
    else
...
RegisterNetEvent('bar-fakeplate:notify', function (...)
    lib.notify(...)
end)