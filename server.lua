...
lib.callback.register('bar-fakeplate:GetPlateIndex', function(source, plate, netid)
...
RegisterNetEvent('bar-fakeplate:InstallFlipper', function(plate)
    local src = source

    if not canInstall(GetPlayerJob(src)) then return end

    local success = exports.ox_inventory:RemoveItem(src, 'fakeplate', 1)
...
RegisterNetEvent('bar-fakeplate:InstallPlateFlipper', function(plate, plateIndex)
    ...
    if RemoveItem(src, 'fakeplate', 1) then
        MySQL.query("UPDATE "..VEHICLE_TABLE.." SET flipper_installed = 1 WHERE plate = ?", { plate })
        vehicles_plate[plate] = plateIndex
        vehicles[plate] = true
        TriggerClientEvent('bar-fakeplate:notify', src, { description = "Plate flipper installed successfully!", type = 'success' } )
    end
end)
...
RegisterUseableItem('fakeplate', function(source, item)
    if HasWhitelistedJob(source) then
        TriggerClientEvent('bar-fakeplate:InstallPlateFlipper', source)
    else
        TriggerClientEvent('bar-fakeplate:notify', source, { type = 'error', description = "You do not have the required job to install the plate flipper!"})
    end
end)